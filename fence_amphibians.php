<?php
  ...
  $amphibianEnclosure = $manager->getEnclosureById(5);
...
<div id="fence" class="container-fluid d-flex justify-content-evenly text-center mt-5" style="position:relative; top: 10vh">
  ...
  <?php foreach ($amphibians as $key => $amphibian) : ?>
    ...
    <img id="animal" src="ANIMAL_ICONS/<?= $amphibian->getIcon()?>" alt="" width="120" height="150"
      data-specie-type="<?= ucfirst($amphibian->getSpecieType()) ?>"
      data-specie-name="<?= ucfirst($amphibian->getSpecieName()) ?>"
      data-size="<?= $amphibian->getSize() ?>"
      data-weight="<?= $amphibian->getWeight() ?>"
      data-age="<?= $amphibian->getAge() ?>"
    >
    </div>

    <audio id="audio<?= $key ?>" src="SOUNDS/<?= $amphibian->getSound() ?>"></audio>
