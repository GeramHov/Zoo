<?php
  ...
  $amphibianEnclosure = $manager->getEnclosureById(2);
  $manager->getAnimalFromEnclosure($amphibianEnclosure);

  $herbivorouses = $amphibianEnclosure->getAnimals();

  $manager = new EnclosureManager($db);
  $enclosures = $manager->getEnclosures();
?>
<body style=" background-image: url('IMAGES/enc.avif');
...
<section id="main" style="background-color: rgb(195, 194, 194, 0.3) ;">
...
<div class="container-fluid text-center">
  <div class="row">
    ...
    <div id="clipart" class="col col-lg-8 col-md-8 col-sm-8">
      ...
      <h1 style="position: absolute; top: 13vh; left:44vw">Herbivorous</h1>
    </div>
...
<?php foreach ($herbivorouses as $key => $herbivorous) : ?>
...
<img id="animal" src="ANIMAL_ICONS/<?= $herbivorous->getIcon()?>" alt="" width="120" height="150"
      data-specie-type="<?= ucfirst($herbivorous->getSpecieType()) ?>"
      data-specie-name="<?= ucfirst($herbivorous->getSpecieName()) ?>"
      data-size="<?= $herbivorous->getSize() ?>"
      data-weight="<?= $herbivorous->getWeight() ?>"
      data-age="<?= $herbivorous->getAge() ?>"
    >
</div>

  <audio id="audio<?= $key ?>" src="SOUNDS/<?= $herbivorous->getSound() ?>"></audio>
...
<script>
  // SIMULATED HOUR
      ...
      function updateTime() {
        // Get the current time from local storage or start at 00:00
        ...
        if (isNighttime) {
          ...
          document.body.style.backgroundImage = "url('IMAGES/enc.avif')";
...
hungerIcons.forEach((icon) => {
  ...
  const hungerthInterval = setInterval(updateHunger, 7000);
});
