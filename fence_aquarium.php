<?php
  ...
  $amphibianEnclosure = $manager->getEnclosureById(7);
  $manager->getAnimalFromEnclosure($amphibianEnclosure);

  $aquariums = $amphibianEnclosure->getAnimals();

  $manager = new EnclosureManager($db);
  $enclosures = $manager->getEnclosures();
?>

<body style=" background-image: url('IMAGES/fence_aqua.jpg');
...
<section id="main" style="background-color: rgb(125, 125, 125, 0.5) ; height: 100vh">
...
<div class="container-fluid text-center">
  <div class="row">
    ...
    <div id="clipart" class="col col-lg-8 col-md-8 col-sm-8">
      ...
      <h1 style="position: absolute; top: 13vh; left:45vw">Aquarium</h1>
    </div>
...
<?php foreach ($aquariums as $key => $aquarium) : ?>
...
<img id="animal" src="ANIMAL_ICONS/<?= $aquarium->getIcon()?>" alt="" width="120" height="150"
      data-specie-type="<?= ucfirst($aquarium->getSpecieType()) ?>"
      data-specie-name="<?= ucfirst($aquarium->getSpecieName()) ?>"
      data-size="<?= $aquarium->getSize() ?>"
      data-weight="<?= $aquarium->getWeight() ?>"
      data-age="<?= $aquarium->getAge() ?>"
    >
</div>

<audio id="audio<?= $key ?>" src="SOUNDS/<?= $aquarium->getSound() ?>"></audio>
  ...
  // SIMULATED HOUR
      ...
      function updateTime() {
        ...
        if (isNighttime) {
          ...
          document.body.style.backgroundImage = "url('IMAGES/fence_aqua.jpg')";
