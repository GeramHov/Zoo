<?php
  ...
  $amphibianEnclosure = $manager->getEnclosureById(1);
  $manager->getAnimalFromEnclosure($amphibianEnclosure);

  $carnivourouses = $amphibianEnclosure->getAnimals();

  $manager = new EnclosureManager($db);
  $enclosures = $manager->getEnclosures();
?>
<body style=" background-image: url('IMAGES/enc_carn.avif');
...
<div class="container-fluid text-center">
  <div class="row">
    ...
    <div id="clipart" class="col col-lg-8 col-md-8 col-sm-8">
      ...
      <h1 style="position: absolute; top: 13vh; left:44vw">Carnivorous</h1>
    </div>
...
<?php foreach ($carnivourouses as $key => $carnivourous) : ?>
...
<img id="animal" src="ANIMAL_ICONS/<?= $carnivourous->getIcon()?>" alt="" width="120" height="150"
      data-specie-type="<?= ucfirst($carnivourous->getSpecieType()) ?>"
      data-specie-name="<?= ucfirst($carnivourous->getSpecieName()) ?>"
      data-size="<?= $carnivourous->getSize() ?>"
      data-weight="<?= $carnivourous->getWeight() ?>"
      data-age="<?= $carnivourous->getAge() ?>"
    >
</div>

  <audio id="audio<?= $key ?>" src="SOUNDS/<?= $carnivourous->getSound() ?>"></audio>
      ...
      function updateTime() {
       ...
       let timeStr = localStorage.getItem("time") || "00:00";
        ...
        if (isNighttime) {
          ...
          document.body.style.backgroundImage = "url('IMAGES/enc_carn.avif')";
...
hungerIcons.forEach((icon) => {
  ...
  const hungerthInterval = setInterval(updateHunger, 2000);
});
