<?php
  ...
  $amphibianEnclosure = $manager->getEnclosureById(4);
  $manager->getAnimalFromEnclosure($amphibianEnclosure);

  $felines = $amphibianEnclosure->getAnimals();

  $manager = new EnclosureManager($db);
  $enclosures = $manager->getEnclosures();
?>
<body style=" background-image: url('IMAGES/enc_felines.jpg');
...
<div class="container-fluid text-center">
  <div class="row">
    ...
    <div id="clipart" class="col col-lg-8 col-md-8 col-sm-8">
      ...
      <h1 style="position: absolute; top: 13vh; left:46vw">Felines</h1>
    </div>
...
<?php foreach ($felines as $key => $feline) : ?>
...
<img id="animal" src="ANIMAL_ICONS/<?= $feline->getIcon()?>" alt="" width="120" height="150"
      data-specie-type="<?= ucfirst($feline->getSpecieType()) ?>"
      data-specie-name="<?= ucfirst($feline->getSpecieName()) ?>"
      data-size="<?= $feline->getSize() ?>"
      data-weight="<?= $feline->getWeight() ?>"
      data-age="<?= $feline->getAge() ?>"
    >
</div>

  <audio id="audio<?= $key ?>" src="SOUNDS/<?= $feline->getSound() ?>"></audio>
...
let timeStr = localStorage.getItem("time") || "00:00";
        ...
        if (isNighttime) {
          ...
          document.body.style.backgroundImage = "url('IMAGES/enc_felines.jpg')";
...
hungerIcons.forEach((icon) => {
  ...
  const hungerthInterval = setInterval(updateHunger, 1000);
});    
